...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, text44):
    ...
    # Use a valid font path or remove fontfile if not needed
    ffmpeg_cmd = (
        f'ffmpeg -i "{filename}" -ss 00:00:10 -vframes 1 '
        f'-vf "drawtext=text=\'{watermark_text}\':fontcolor=white:fontsize=24'
        f':x=(w-tw)/2:y=(h-th)/2" '  # Removed fontfile and other options for testing
        f'-y "{thumbnail_output}"'
    )
    ...
    try:
        ...
        await m.reply_text(f"Error generating thumbnail: {e.stderr}")
        return
    ...
    try:
        thumbnail = thumbnail_output if thumb == "/d" else thumb
        # Rest of your code to send the video          
        except Exception as e:
